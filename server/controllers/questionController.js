...
module.exports = {
    ...
    showAll: (req, res) => {
        Question.find({}).populate('user', '_id first_name last_name').populate('upvote downvote').then((result) => {
    ...
    delete: (req, res) => {
        ...
        Question.deleteOne({_id: req.params.id, user: req.decoded.id}).then((result) => {
    ...
    findById: (req, res) => {
        Question.findById(req.params.id).populate('user', '_id first_name last_name').populate('upvote downvote').then((result) => {
            res.status(200).json(result);
        }).catch((err) => {
            res.status(400).json(err.message);
        });
